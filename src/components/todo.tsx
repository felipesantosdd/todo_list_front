...
const Todo = ({ id, title, text, color, isFavorited }: todoType) => {
    ...
    return (
        <div
            className={`w-[390px] h-[430px] rounded-3xl m-[15px] bg-white shadow-lg  `}
        ...
        >
            <div className="w-[100%] h-[50px] flex items-center  pl-5 pr-5 border-b flex-row justify-between">
                ...
                <button
                    onClick={async () =>
                        await UpdateTodos(id, { isFavorited: !isFavorited })
                    }
                >
                    <Image
                        src={isFavorited ? favoriteTrue : favoriteFalse}
                        width={20}
                        height={20}
                        alt={"star"}
                    />
